...
**Key Features of the [DirectWrite font rendering] (https://docs.microsoft.com/en-us/windows/win32/direct2d/direct2d-and-directwrite#glyph-rendering)
...
3. **Composites: Hinting code editing required** (Composite, that use(USEMYMETRICS, OFFSETS and positioning code). As of writing the code generated by the Autohinter is does not work for Variable fonts. The code calls Function 86, which uses an ‘outline measured distance as a reference for positioning the accent in the y-direction. This works for static fonts, where there is only one measured distance. In Varaible fonts the distance between the accent and base glyph can vary. The currently generated code positions the accent correctly for the default instance only. If the measured outline distance between the base glyph and the accent is the same across the variation space, this code can be used. if the accent ensiled distance varies, custom code is needed.)
